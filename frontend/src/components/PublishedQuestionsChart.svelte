<!-- frontend/src/components/PublishedQuestionsChart.svelte -->
<script>
  ...
  async function fetchData() {
    try {
      ...
      const { data, ok, status } = await get('published-stats');
      ...
      maxValue = Math.max(...chartData.map(item => item.count), 1); // حداقل 1 برای جلوگیری از تقسیم بر صفر
